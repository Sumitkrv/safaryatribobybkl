...
export default function ExploreIndia() {
  ...
  const toggleState = (stateIdx) => setExpandedStates(prev => ({ ...prev, [stateIdx]: !prev[stateIdx] }));
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: '#F8FAFC' }}>
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }}
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button onClick={() => { setSubmitState('idle'); setSelectedPlaces([]); }}
                ...
                <div className="relative">
                  ...
                  <div className="space-y-10 relative z-10">
                    {filteredData.map((state, stateIdx) => {
                      ...
                      return (
                        <motion.div key={state.state} ref={el => stateRefs.current[originalIdx] = el}
                          variants={fadeUp} custom={stateIdx} initial="hidden" whileInView="visible"
                          viewport={{ once: true, margin: '-50px' }} className="scroll-mt-28">
                          {/* Timeline dot */}
                          <div className="hidden md:flex items-center gap-4 mb-4 pl-[18px] md:pl-[28px]">
                            <div className="w-5 h-5 rounded-full border-[3px] border-white shadow-md z-10 shrink-0" style={{ background: accentColor }} />
                          ...
                          <div className="md:ml-[52px]">
                            <div className="rounded-[2rem] overflow-hidden group/card" style={{ boxShadow: '0 8px 40px -12px rgba(15,23,42,0.12)' }}>
                              {/* Hero image */}
                              <div className="relative cursor-pointer" onClick={() => toggleState(originalIdx)}>
                                <div className="relative h-[280px] md:h-[320px] overflow-hidden">
                                  <img src={heroImg} alt={state.state} className="w-full h-full object-cover transition-transform duration-[1.2s] group-hover/card:scale-105" />
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                                      ...
                                      <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <motion.div animate={{ rotate: isStateExpanded ? 180 : 0 }}
                              ...
                              <AnimatePresence>
                                {isStateExpanded && (
                                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                                    exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
                                    ...
                                    <div className="bg-[#FAFBFC] border-t border-slate-100">
                                      ...
                                      {/* ── Expanded city: interactive attraction cards ── */}
                                      {state.cities.map((city, cityIdx) => {
                                        const isExpanded = expandedCities[`${originalIdx}-${cityIdx}`];
      ...
      <div ref={formRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <div className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold"
            style={{ background: 'linear-gradient(135deg, #00CFC8, #0EA5E9)', color: '#fff' }}>
            <Send className="w-4 h-4" /> Send Trip Inquiry
          </div>
          <div className="flex-1 h-px" style={{ background: '#E2E8F0' }} />
        </div>
        ...
        {selectedPlaces.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <div>
              <p className="text-sm font-semibold text-orange-700">No attractions selected yet</p>
              <p className="text-xs text-orange-600 mt-0.5">Scroll up, open a state → city, and tap any attraction card to add it to your trip.</p>
            </div>
          </motion.div>
        ) : (
          <div className="rounded-2xl p-5 mb-8" style={{ background: '#F0FDFA', border: '1px solid #99F6E4' }}>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="p-6">
                <TripSummaryCard selectedPlaces={selectedPlaces} removePlace={removePlace} destination={destinationLabel}
                  onScrollToForm={() => { setMobileSummaryOpen(false); setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth' }), 300); }} />
              </div>
            </motion.div>
          </>
        )}
